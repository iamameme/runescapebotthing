...
def mouse_bez(init_pos, fin_pos, deviation, speed):
    ...
    print('mosuebez')
    #time parameter
    ts = [t/(speed * 100.0) for t in range(speed * 100)]
...
def move(mouse_points, draw = False, rand_err = True):
    ...
    if type(mouse_points) is not str:
        mouse_points = [round(mouse_points[0]), round(mouse_points[1])]
    # mouse_points = [[round(v) for v in range(round(x))] if type(x) is not str else x for x in mouse_points]
    print(mouse_points)
    sleep(.01)
    if mouse_points == 'click':
        a = 'mouse.click(Button.left, 1)'
    else:
        mouse.position = (mouse_points[0],mouse_points[1])
